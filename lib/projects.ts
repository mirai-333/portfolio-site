...
const beyondTheTorii: Project = {
  ...
  process: [
    ...
    { title: '02. Unity / XR基盤構築', body: 'Unityをメイン開発環境として、XR Interaction Toolkitを用いたVR基盤を構築しました。Meta Questで動作するよう、Grab Interactionを組み込み、各シーンで共通して使える操作を整えました。初めてVRを使うユーザーも対象だったため、操作ごとに異なるルールを作るのではなく、できるだけ一貫したインタラクションに統一しました。' },
    { title: '03. Navigation / Scene Flow', body: '体験は鳥居から始まり、手水、参道、レストラン、抹茶づくり体験、着物体験を経て、再び鳥居から退出する流れで構成しました。各体験では、必要な手順を細かく案内することで、ユーザーが次に何をすればよいか迷わず進められるようにしました。また、次のエリアへ向かう矢印を常に表示し、ユーザーが次にどこへ行けばよいかを理解できるようにしました。' },
